...
import '../data_sources/notification_datasource.dart';
...
class NotificationRepositoryImpl implements NotificationRepository{
  ...
  NotificationRepositoryImpl(this.datasource, this._networkInfo);

  @override
  Future<Result<DefaultSuccessModel, FailureModel>> updateFCMToken(String token) async {
    if (await _networkInfo.isConnected) {
      try {
        final response = await datasource.updateFCM(token);
  ...
  @override
  Future<Result<PaginationDataModel<NotificationEntity>, FailureModel>> getNotification(PaginationParams params) async {
    if (await _networkInfo.isConnected) {
      try {
        final response = await datasource.getNotifications(params.page);
        if (response.response.statusCode == 200 ||
            response.response.statusCode == 201) {
          return Success(response.data);
        } else {
