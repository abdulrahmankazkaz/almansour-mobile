...
import 'package:mac_mobile/features/main/entities/image_entity/image_entity.dart';
...
class MainRepositoryImplementation implements MainRepository {
  ...
  MainRepositoryImplementation(this.datasource,this._networkInfo);


  @override
  Future<Result<DefaultDataModel<ImageEntity>, FailureModel>> uploadImage(XFile image) async {
    if (await _networkInfo.isConnected) {
      try {
        final response = await datasource.uploadImage(File(image.path));
  ...
  @override
  Future<Result<DefaultDataModel<List<BasicModel>>, FailureModel>> getOrderTypes() async {
    if (await _networkInfo.isConnected) {
      try {
        final response = await datasource.getOrderTypes();
        if (response.response.statusCode == 200 ||
            response.response.statusCode == 201) {
          return Success(response.data);
        } else {
  ...
  @override
  Future<Result<DefaultDataModel<List<BasicModel>>, FailureModel>> getPaymentMethods() async {
    if (await _networkInfo.isConnected) {
      try {
