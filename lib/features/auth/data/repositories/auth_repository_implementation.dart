...
class AuthRepositoryImplementation implements AuthRepository {
  ...
  @override
  Future<Result<DefaultSuccessModel, FailureModel>> login(
      String phoneNumber) async {
    if (await _networkInfo.isConnected) {
      try {
        final response = await datasource.login(phoneNumber);
        if (response.response.statusCode == 200) {
          LoggerService().logDebug(message: 'status code 200 ${response.data}');
  ...
  Future<Result<AuthVerify, FailureModel>> verifyMobileNumber(
    ...
    if (await _networkInfo.isConnected) {
      try {
        final response = await datasource.verifyMobileNumber(
            parameters.mobileNumber, parameters.code,getIt<AppPreferences>().userPreferences.getFCMToken());
        if (response.response.statusCode == 200) {
        ...
        if (e.type == DioExceptionType.cancel) {
          return Error(FailureModel(message: DioExceptionType.cancel.toString()));
        }
        else if(e.type == DioExceptionType.connectionTimeout || e.type == DioExceptionType.receiveTimeout || e.type == DioExceptionType.sendTimeout){
          return Error(FailureModel(message: LocaleKeys.dioError_timeOut.tr()));
        }
  ...
  @override
  Future<Result<DefaultSuccessModel, FailureModel>> logout() async {
    if (await _networkInfo.isConnected) {
      try {
  ...
  Future<Result<DefaultSuccessModel, FailureModel>> resendOTP(
    ...
    if (await _networkInfo.isConnected) {
      try {
        final response = await datasource.resendOTP(phoneNumber);
        if (response.response.statusCode == 200) {
          LoggerService().logDebug(message: 'status code 200 ${response.data}');
          return Success(response.data);
        } else {
