...
import '../../domain/repositories/customer_car_repository.dart';
import '../../entities/parameters/customer_car_parameters.dart';
import '../datasources/customer_car_datasource.dart';

class CustomerCarRepositoryImplementation implements CustomerCarRepository {
  final CustomerCarDatasource datasource;

  final NetworkInfo _networkInfo;
  CustomerCarRepositoryImplementation(this.datasource, this._networkInfo);
  ...
  Future<Result<CustomerCar, FailureModel>> setCustomerCar(
    ...
    if (await _networkInfo.isConnected) {
      try {
        final response = await datasource.setCustomerCar(parameters);
  ...
  @override
  Future<Result<CustomerCar?, FailureModel>> getCustomerCar() async {
    if (await _networkInfo.isConnected) {
      try {
        final response = await datasource.getCustomerCar();
        if (response.response.statusCode == 200 ||
            response.response.statusCode == 201) {
          return Success(response.data.data);
        } else {
  ...
  @override
  Future<Result<CustomerCar, FailureModel>> updateOdometer(OdometerParameters parameters) async {
    if (await _networkInfo.isConnected) {
      try {
        final response = await datasource.updateOdometer(parameters);
        if (response.response.statusCode == 200 ||
            response.response.statusCode == 201) {
          return Success(response.data.data);
        } else {
